...
transmitd = ss(A, B, C, D, 2,'InputDelay', [0, 180, 0, 0, 0, 0]);

transfer = tf(transmit);
...
utf = [ones(1,2000)*Fh;ones(1,2000)*Fcin;ones(1,2000)*Fd;ones(1,2000)*Th;ones(1,2000)*Tc;ones(1,2000)*Td];
...
hold on
%if addconst == 1
%    lsim(transfer, u, t, [V0, T0, -a/sqrt(sqrt(C0))*(sqrt(sqrt(V0))-0.25*V0/sqrt(sqrt(V0^3)))])
%else
%    lsim(transfer, u, t, [V0, T0])
%end
