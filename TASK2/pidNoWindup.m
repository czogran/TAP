...
UseBackCalculation = false; % enable/disable anti-windup (back calculation)
G=tf(transmit);
KpFh = 0.5; KiFh = 0.001; % PI controller gains (parallel)
KpFc = -20; KiFc = -0.01; % PI controller gains (parallel)


PID_Fh = tunablePID('PID_Fh','pi');  
PID_Fh.u = 'e(1)'; PID_Fh.y = 'Fh';
PID_Fh.Kp.Value=KpFh; PID_Fh.Ki.Value=KiFh;

PID_Fc = tunablePID('PID_Fc','pi');  
PID_Fc.u = 'e(2)'; PID_Fc.y = 'Fc';
PID_Fc.Kp.Value=KpFc; PID_Fc.Ki.Value=KiFc;
...
for ct=1:N
    ...
    controls;
    x=x1(2,:);
    cVector(ct,1)=max(min(controls(2,1),UB),LB);
    cVector(ct,2)=max(min(controls(2,2),UB),LB);
...
%         x(1) = x(1) + (KiFh*e(1) + (cVector(ct,1)-x(1))/tau)*Ts;
%         x(2) = x(2) + (KiFh*e(2) + (cVector(ct,2)-x(2))/tau)*Ts;
% 
